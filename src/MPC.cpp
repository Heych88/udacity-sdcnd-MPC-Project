 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 2; t++) {
      ...
      fg[2 + cte_start + t] = cte1 - (fx0 - y0 + (v0 * CppAD::sin(epsi0) * dt));
      AD<double> psi_des = coeffs[1] + 2 * coeffs[2] * x0 + 3 * coeffs[3] * x0 * x0;
      fg[2 + epsi_start + t] = epsi1 - (psi0 - psi_des + (v0 / Lf * delta0 * dt));
    }
...
std::tuple<vector<double>, vector<double>, vector<double>> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  vector<double> path_x(solution.x[x_start], solution.x[x_start + N]);
  vector<double> path_y(solution.x[y_start], solution.x[y_start + N]);
